...
func (_q *StreamerQuery) QueryPlatform() *PlatformQuery {
...
func (_q *StreamerQuery) QueryFollowings() *UserFollowingQuery {
...
func (_q *StreamerQuery) WithPlatform(opts ...func(*PlatformQuery)) *StreamerQuery {
	query := (&PlatformClient{config: _q.config}).Query()
...
func (_q *StreamerQuery) WithFollowings(opts ...func(*UserFollowingQuery)) *StreamerQuery {
	query := (&UserFollowingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(streamer.FieldDeleteAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *StreamerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Streamer, error) {
	...
	if query := _q.withPlatform; query != nil {
		if err := _q.loadPlatform(ctx, query, nodes, nil,
			func(n *Streamer, e *Platform) { n.Edges.Platform = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withFollowings; query != nil {
		if err := _q.loadFollowings(ctx, query, nodes,
			func(n *Streamer) { n.Edges.Followings = []*UserFollowing{} },
			func(n *Streamer, e *UserFollowing) { n.Edges.Followings = append(n.Edges.Followings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *StreamerQuery) loadPlatform(ctx context.Context, query *PlatformQuery, nodes []*Streamer, init func(*Streamer), assign func(*Streamer, *Platform)) error {
...
func (_q *StreamerQuery) loadFollowings(ctx context.Context, query *UserFollowingQuery, nodes []*Streamer, init func(*Streamer), assign func(*Streamer, *UserFollowing)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userfollowing.FieldStreamerID)
	}
	query.Where(predicate.UserFollowing(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(streamer.FollowingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *StreamerQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withPlatform != nil {
			_spec.Node.AddColumnOnce(streamer.FieldPlatformID)
		}
	}
